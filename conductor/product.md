...
A Claude Code powerline/statusline tool written entirely in Go. Similar to `claude-limitline` (TypeScript/Node.js) but following Go idioms and conventions. Installable via `go run github.com/rbarcante/conductor-powerline@latest`. Displays Claude Code API usage (5-hour block, 7-day rolling), git branch info, active model, and more — targeting Claude Code power users who take AI-assisted coding seriously.
...
- Feature parity with claude-limitline (all segments, themes, config)
