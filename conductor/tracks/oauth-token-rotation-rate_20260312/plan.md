...
- [ ] Task: Define `TokenCredentials` struct in `internal/oauth/types.go`
  - [ ] Add struct with `AccessToken` and `RefreshToken` fields
- [ ] Task: Expand `claudeAiOAuthEntry` in `credfile.go` to parse `refreshToken`
...
- [ ] Task: Update `usage.go` to use `credentialsGetter` instead of `tokenGetter`
...
- [ ] Task: Implement `RefreshOAuthToken()` in `client.go`
...
- [ ] Task: Write comprehensive tests using `httptest`
...
- [ ] Task: Create `internal/oauth/rotatedtoken.go`
...
- [ ] Task: Write tests in `rotatedtoken_test.go`
...
- [ ] Task: Integrate with `GetCredentials()` — check rotated token first
...
- [ ] Task: Modify 429 handler in `usage.go`
...
- [ ] Task: Write tests for all rotation paths
