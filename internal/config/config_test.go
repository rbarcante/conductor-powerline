...
func TestDefaultConfig(t *testing.T) {
	...
	expectedOrder := []string{"directory", "git", "model", "block", "weekly", "context", "conductor", "conductor_workflow"}
...
func TestMergeConfigPartialOverride(t *testing.T) {
	...
	if len(merged.SegmentOrder) != 8 {
		t.Errorf("expected 8 segment order items from base, got %d", len(merged.SegmentOrder))
	}
}
...
func TestLoadNoFiles(t *testing.T) {
	...
	if len(cfg.SegmentOrder) != 8 {
		t.Errorf("expected 8 default segments, got %d", len(cfg.SegmentOrder))
	}
}
